...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/02/17 07:09:44 by rbardet-          #+#    #+#             */
/*   Updated: 2025/02/17 13:59:25 by throbert         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	shell->fd = open(shell->cmd[file_pos], O_CREAT | O_TRUNC | O_WRONLY, 0777);
	if (access(shell->cmd[file_pos], X_OK) == -1)
	{
...
static t_shell	*output_exec(t_shell *shell, int fd, t_ast *ast)
{
	pid_t	pid;
	int		status;
...
static t_shell	*get_flag(t_shell *shell, int i, int file_pos, t_ast *ast)
{
	...
	if (shell->exit_code == 1)
		return(shell);
	shell = output_exec(shell, shell->fd, ast);
	free_tab(shell->cmd_flag);
	return(shell);
}
...
int	redirect_output(t_shell *shell, t_ast *ast)
{
	...
	shell->cmd_flag = NULL; // Add this line
